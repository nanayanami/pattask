
# Pattask

<img width="130％" alt="ロゴ" src="https://github.com/user-attachments/assets/54d07db2-860c-4c8e-aa06-e509fe16d3ff" /><br>


<img width="528" height="50" alt="image" src="https://github.com/user-attachments/assets/5eb9f108-38c9-44ea-9c8d-9d7817c3127f" />


# 概要
「あれ、この案件は目標に対しどこまで進んでいるのだろう…？」という、ビジネスマンが一度は経験する心理的不安を解決するために開発しました。
 日々のタスクを整理し、目標までの道のりを「見える化」することで、ユーザーは適切なスケジュールを組むことができるようになります。
 さらに、チーム内での進捗共有機能も搭載しました。
 よって、「誰が、どこで、止まっているか」がひと目でわかるため、チームはスムーズな連携を手に入れることができます。

## サイトテーマ
「仕事の迷子をゼロにする。パッとタスクを進捗させるツール、Pattask」

## テーマを選んだ理由
タスク進捗管理は必ず必要なスキルですが新しい業務内容に取り組む時に、正しい進捗ができているのか分からなくなってしまうことがありました。本アプリで整理し、客観的に捉えなおしたいと思うようになりました。
また、チームメンバーの案件内容を確認できたり、課題解決策を知ることができるため、ケースワークに繋がります。
更に、第三者からの確認ができるようにすることで誰１人取り残さない体制の構築を目指します。

### 【きっかけ】
私自身が、現在の業務において、1人あたり月15社の掲出に向けたマネジメントに追われるなかで、自分自身が最終ゴールに対するの現在地を見失った経験がありました。
そこで、状況を打破するために常にゴールから逆算して課題を構造化し、チームが互いの異変に即座に気づける環境をシステムで構築しようと考えるようになりました。

### 【問題点】
- 進捗の「ブラックボックス化」による不安
- 知識や解決策が共有されず、各自がゼロから悩む
- 孤立による挫折

### 【解決策】
- 未知の業務に対する「進捗への不安」を解消<br>
　状況を客観的に整理しなおすことで、自分の進め方が正しいかを冷静に判断できます。
- 他者の進捗や解決策から学び、ケースワークができる<br>
　第三者の状況をナレッジとして共有し、チーム全体の対応力を引き上げます。
- チーム全員で完遂を目指す<br>
　周囲が状況を把握しフォローし合える状態を作ることで、誰も取り残さない体制を構築します。

# ターゲットユーザー
- マネジメント業務や提案業務を行う社会人
- 課題整理を行いたい人
- チーム制作を行っている人
- 学校行事の実行委員やチームプレゼン等を行う学生

# 利用方法
- ユーザー新規登録を行う、またはログインする<br>
  <img width="465" height="281" alt="image" src="https://github.com/user-attachments/assets/2b8006a4-93d3-40a9-9ea3-1a539300d2cb" />

- チーム、カテゴリー、投稿でタスクを分類分けする<br>
  <img width="546" height="185" alt="image" src="https://github.com/user-attachments/assets/7fba603e-257c-4e7e-bee7-f22a1a14d303" />


- タスクの進捗状況を投稿一覧や詳細で管理/メンバー同士の案件における成功事例やリカバリー策の共有<br>
  <img width="460" height="279" alt="image" src="https://github.com/user-attachments/assets/70d7a324-04bf-41d1-9b10-1b2d3fe4c6d4" />

  
- 表現力豊かなリッチテキストエディタ搭載<br>
  <img width="458" height="284" alt="image" src="https://github.com/user-attachments/assets/77e3b818-1266-4c44-8b7b-0e3c00951abd" />
  
- 自身の仕事の進捗状況や、課題整理が可能/コメント機能を付けることでフォロー体制を整える<br>
  <img width="463" height="275" alt="image" src="https://github.com/user-attachments/assets/81b4c2cf-3475-4619-abf1-3b5c0124f0b8" />


- マイページ、ユーザー機能により各自が受け持つ仕事内容を整理<br>
  <img width="461" height="271" alt="image" src="https://github.com/user-attachments/assets/62c6cb02-18b2-47b7-8cad-20bde31e5b53" />

- ブックマーク機能やフォロー機能により、チームメンバーやマネージャーの進捗状況把握をスムーズにする<br>
  <img width="466" height="354" alt="image" src="https://github.com/user-attachments/assets/9ca44c8e-9bc5-44a7-a2b8-a053ed4f2273" />

- チーム/カテゴリー/投稿単位で検索機能追加<br>
  <img width="356" height="146" alt="image" src="https://github.com/user-attachments/assets/fff62e2d-fb28-42f2-915c-99703b16bec5" />

  
# 機能一覧
- 基本的なCRUD機能
- チーム、カテゴリー、投稿作成機能
- ユーザー認証機能(devise)
- 進捗状況の可視化
- 進捗完了機能
- ブックマーク機能
- コメント機能   
- 下書き機能  
- 検索機能    
- バリデーション  
- ページネーション 

# 開発環境
- Windows11 + Git Bash
- 言語：HTML、CSS、JavaScript、Ruby
- フレームワーク：Ruby on Rails
- JSライブラリ：jQuery
- データベース：PostgreSQL
